...
class DoctorAvailabilitySerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # ✅ use get_or_create to match the views — no crash if profile missing
...
class DoctorProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Doctor
        fields = ['bio', 'contact', 'years_of_experience', 'profile_picture', 'specialty_id', 'specialty_name']
...
class AppointmentStatusSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        # ✅ 'completed' added — doctors can mark appointments as done
        # ✅ 'rejected' maps to 'cancelled' since model has no rejected field
        allowed = ['confirmed', 'cancelled', 'completed', 'rejected']
        if value not in allowed:
            raise serializers.ValidationError(
                f"Status must be one of: confirmed, cancelled, completed, rejected"
            )
        if value == 'rejected':
            return 'cancelled'   # map to what the model actually stores
        return value